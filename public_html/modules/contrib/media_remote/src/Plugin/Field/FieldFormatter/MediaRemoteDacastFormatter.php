...
/**
 * Plugin implementation of the 'media_remote_dacast' formatter.
 *
 * @FieldFormatter(
 *   id = "media_remote_dacast",
 *   label = @Translation("Remote Media - Dacast"),
 *   field_types = {
 *     "string"
 *   }
 * )
 */
class MediaRemoteDacastFormatter extends MediaRemoteFormatterBase {

  /**
   * {@inheritdoc}
   */
  public static function getUrlRegexPattern() {
    return '/^https:\/\/iframe\.dacast\.com\/vod\/(-*.*)/';
  }
  ...
  public static function getValidUrlExampleStrings(): array {
    return [
      'https://iframe.dacast.com/vod/[video-id]',
    ];
  }
  ...
  public static function deriveMediaDefaultNameFromUrl($url) {
    ...
    if (!empty($matches[1][0])) {
      return t('Dacast video from @url', [
        '@url' => $url,
      ]);
    }
